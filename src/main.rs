...
// Your hot-reloadable function (keep #[inline(never)] for best patching)
#[unsafe(no_mangle)]
#[inline(never)]
pub fn my_hot_function(a: &Series, b: &Series) -> Option<f64> {
...
fn main() {
    ...
    // Show initial address (optional, for debugging)
    let hotfn = HotFn::current(my_hot_function);
    println!("my_hot_function ready at {:?}", hotfn.ptr_address());
...
fn app() -> Element {
    let mut last_addr = use_signal(|| HotFn::current(my_hot_function).ptr_address());
    ...
    let mut run_tests = move || {
        ...
        // Run the logic directly — no catch_unwind needed
        let pos = {
            let a = Series::new("a".into(), &[1, 2, 3, 4, 5]);
            let b = Series::new("b".into(), &[1, 2, 3, 4, 5]);
            call(|| my_hot_function(&a, &b)).unwrap_or(f64::NAN)
        };
        ...
        // Detect patch safely (no mutation inside catch_unwind)
        let current_addr = HotFn::current(my_hot_function).ptr_address();
        let patch_note = if current_addr != *last_addr.read() {
            last_addr.set(current_addr);  // This is safe here
        ...
        let output = format!(
            ...
            Constant series      : {:.10}\n\
            \n\
            All assertions passed!{patch_note}",
            pos, neg, zero
        );
    ...
    rsx! {
        div {
            ...
            p {
                ...
                "Tip: Open src/corr.rs or my_hot_function to edit and trigger hot-reload."
            }
        }
    }
}